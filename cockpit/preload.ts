...
contextBridge.exposeInMainWorld("devloop", {
  ...
  nativeSystemLogs: (on: boolean) => ipcRenderer.invoke("devloop:nativeSystemLogs", on),
  ...
  onMenu: (cb: (action: string) => void) => {
    ...
    ipcRenderer.on("devloop:menu", handler);
    return () => ipcRenderer.removeListener("devloop:menu", handler);
  },
