...
try {
  ...
  check("tools/list over HTTP exposes the shared tool layer", tools.tools.some((t) => t.name === "browser_snapshot") && tools.tools.some((t) => t.name === "get_network"));
  ...
  await a.callTool({ name: "browser_navigate", arguments: { url: "data:text/html,<script>console.log('daemon-shared-marker')</script>" } });
  await sleep(900);
  const seenByB = textOf((await b.callTool({ name: "get_logs", arguments: { grep: "daemon-shared-marker", limit: 50 } })) as { content: { text?: string }[] });
  check("a second client sees the first client's browser activity (shared backend)", seenByB.includes("daemon-shared-marker"), seenByB.slice(0, 80));

  // clear_logs from A is visible to B too (one buffer, not per-session).
  await a.callTool({ name: "clear_logs", arguments: {} });
  await sleep(200);
  const afterClear = textOf((await b.callTool({ name: "get_logs", arguments: { grep: "daemon-shared-marker", limit: 50 } })) as { content: { text?: string }[] });
